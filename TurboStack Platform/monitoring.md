...
![Monitoring dashboard|400](../img/turbostackapp/monitoring/ts_monitoring_dashboard.png)

Alerts on the dashboard are grouped into **four categories**, each focused on a specific type of problem:

### Alerts

The **Alerts** panel lists the most important **server and application checks** that are currently in a non-OK state. Entries are **sorted by criticality**, first by **priority** (from **P1** - most critical - to **P4** - least critical), and within each priority by **status**: `CRITICAL` first, then `WARNING`, and finally `UNKNOWN`.
...
![Alerts panel|400](../img/turbostackapp/monitoring/ts_monitoring_alerts.png)
