...
phpMyAdmin is easily accessible via the [TurboStack Platform](https://my.turbostack.app "TurboStack Platform"). To locate the link, go to `Hosts` > `Manage` > `Applications` > `Gear icon (x2)` > `Database info`. From there, click the Go to phpMyAdmin link.
Once there, you will encounter a password prompt. Enter your SSH user credentials (application name and password) to proceed.
